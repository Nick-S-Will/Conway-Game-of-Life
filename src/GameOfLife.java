...
public class GameOfLife {
	...
	public GameOfLife() {
		...
		int grid = Math.round(800 / height);
		...
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				String xTxt = Integer.toString(x), yTxt = Integer.toString(y);
				while (xTxt.length() < 2) xTxt = "0" + xTxt;
				while (yTxt.length() < 2) yTxt = "0" + yTxt;

				JButton clone = new JButton(xTxt + yTxt);
